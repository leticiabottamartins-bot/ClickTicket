...
const SELECT_INGRESSO = `SELECT i.id, i.usuario_id, u.nome AS nome_usuario, s.nome AS nome_show, i.lote_id, li.tipo, i.preco_pago
    FROM ingresso i
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const r6 = await db.query("SELECT * FROM ingresso WHERE lote_id = $1 AND id <> $2", [lote_id, id])
        if (quantidade - r6.rowCount < 1) {
            throw new Error("Ingressos esgotados")
        }

        const r3 = await db.query("UPDATE ingresso SET usuario_id = $1, lote_id = $2, preco_pago = $3 WHERE id = $4", [usuario_id, lote_id, preco_pago, id])
        if (!r3.rowCount) {
            throw new Error("Não foi possível editar o ingresso")
        }
        const r4 = await db.query(`${SELECT_INGRESSO} WHERE i.id = $1`, [id])
