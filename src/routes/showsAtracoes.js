...
router.post("/", async (req, res) => {
    try {
        ...
        const atracaoIdNum = Number(atracaoId);

        if (!Number.isInteger(showIdNum)) {
            throw new Error("ID do show inválido");
        }

        if (!Number.isInteger(atracaoIdNum)) {
            throw new Error("ID da atração inválido");
        }
        const r1 = await db.query("SELECT data FROM show WHERE id = $1", [showIdNum])
        if (!r1.rowCount) {
        ...
        const r3 = await db.query("INSERT INTO show_atracao (show_id, atracao_id) VALUES ($1, $2) RETURNING *", [showIdNum, atracaoIdNum]);
        if (!r3.rowCount) {
            throw new Error("Atracoes nao adicionadas ao show");
        }
        const r4 = await db.query("SELECT s.nome AS show_nome, s.id AS show_id, a.nome AS atracao_nome, a.id AS atracao_id FROM show s JOIN show_atracao sa ON sa.show_id = s.id JOIN atracao a ON sa.atracao_id = a.id WHERE sa.show_id = $1 AND sa.atracao_id = $2", [showIdNum, atracaoIdNum])
        return res.status(201).json({ msg: "Atrações adicionadas ao show!", ID_show: r4.rows[0].show_id, nome_show: r4.rows[0].show_nome, ID_atracao: r4.rows[0].atracao_id, nome_atracao: r4.rows[0].atracao_nome });
    } catch (error) {
        return res.status(400).json({ msg: error.message })
    }
})
...
router.put("/:showId/:atracaoIdVelha", async (req, res) => {
    try {
        ...
        const r4 = await db.query("SELECT s.id AS show_id, s.nome AS show_nome, a.id AS atracao_id, a.nome AS atracao_nome FROM show s JOIN show_atracao sa ON s.id = sa.show_id JOIN atracao a ON sa.atracao_id = a.id WHERE sa.show_id = $1 AND sa.atracao_id = $2 ", [showId, atracaoId])
...
router.delete("/:showId/:atracaoId", async (req, res) => {
    try {
        ...
        return res.status(200).json({ msg: "Atração cancelada no show!" })
