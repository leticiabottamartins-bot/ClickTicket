...
router.get("/", async (req, res) => {
    try {
        const r = await db.query("SELECT li.show_id, s.id, s.nome AS nome_show, li.tipo, li.preco, li.quantidade FROM lote_ingresso li JOIN show s ON s.id = li.show_id")
        if (!r.rowCount) {
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const r = await db.query("SELECT li.show_id, s.id, s.nome AS nome_show, li.tipo, li.preco, li.quantidade FROM lote_ingresso li JOIN show s ON s.id = li.show_id WHERE id = $1", [id])
        if (!r.rowCount) {
...
router.post("/", async (req, res) => {
    try {
        ...
        const r = await db.query("INSERT INTO lote_ingresso (show_id, tipo, preco, quantidade) VALUES ($1, $2, $3, $4) RETURNING *", [show_id, tipo, preco, quantidade])
        if (!r.rowCount) {
